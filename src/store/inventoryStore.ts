...
interface InventoryState {
  ...
  addProduct: (p: Omit<Product, 'id' | 'reorderFlag'>) => void;
  updateProduct: (id: string, p: Partial<Omit<Product, 'id' | 'reorderFlag'>>) => void;
...
// Derives reorderFlag from current stock vs threshold
function withReorderFlag(p: Product): Product {
  return { ...p, reorderFlag: p.stock < p.minStockThreshold };
}

function applyReorderFlags(products: Product[]): Product[] {
  return products.map(withReorderFlag);
}
...
const dummyProducts: Product[] = applyReorderFlags([
  {
    id: '1', name: 'Steel Beam A36', sku: 'STL-001', category: 'Raw Materials',
    unitOfMeasure: 'pcs', stock: 245, location: 'WH/Stock/A1',
    minStockThreshold: 50, reorderFlag: false,
    stockLocations: [
      { warehouseId: 'wh1', warehouseName: 'Main Warehouse',    locationId: 'loc1', locationName: 'Stock/A1', quantity: 200 },
      { warehouseId: 'wh2', warehouseName: 'Secondary Storage', locationId: 'loc9', locationName: 'Stock/A1', quantity: 45  },
    ],
  },
  {
    id: '2', name: 'Copper Wire 2.5mm', sku: 'COP-012', category: 'Raw Materials',
    unitOfMeasure: 'meters', stock: 1200, location: 'WH/Stock/A2',
    minStockThreshold: 200, reorderFlag: false,
    stockLocations: [
      { warehouseId: 'wh1', warehouseName: 'Main Warehouse', locationId: 'loc2', locationName: 'Stock/A2', quantity: 1200 },
    ],
  },
  {
    id: '3', name: 'Circuit Board v3', sku: 'PCB-003', category: 'Components',
    unitOfMeasure: 'pcs', stock: 3, location: 'WH/Stock/B1',
    minStockThreshold: 20, reorderFlag: false,
    stockLocations: [
      { warehouseId: 'wh1', warehouseName: 'Main Warehouse', locationId: 'loc4', locationName: 'Stock/B1', quantity: 3 },
    ],
  },
  {
    id: '4', name: 'Hydraulic Pump HP-50', sku: 'HYD-050', category: 'Machinery',
    unitOfMeasure: 'units', stock: 8, location: 'WH/Stock/C1',
    minStockThreshold: 5, reorderFlag: false,
    stockLocations: [
      { warehouseId: 'wh1', warehouseName: 'Main Warehouse', locationId: 'loc6', locationName: 'Stock/C1', quantity: 8 },
    ],
  },
  {
    id: '5', name: 'Aluminum Sheet 2mm', sku: 'ALU-020', category: 'Raw Materials',
    unitOfMeasure: 'sheets', stock: 67, location: 'WH/Stock/A3',
    minStockThreshold: 30, reorderFlag: false,
    stockLocations: [
      { warehouseId: 'wh1', warehouseName: 'Main Warehouse',    locationId: 'loc3', locationName: 'Stock/A3', quantity: 50 },
      { warehouseId: 'wh2', warehouseName: 'Secondary Storage', locationId: 'loc10', locationName: 'Stock/A3', quantity: 17 },
    ],
  },
  {
    id: '6', name: 'Servo Motor SM-100', sku: 'SRV-100', category: 'Components',
    unitOfMeasure: 'units', stock: 2, location: 'WH/Stock/B2',
    minStockThreshold: 10, reorderFlag: false,
    stockLocations: [
      { warehouseId: 'wh1', warehouseName: 'Main Warehouse', locationId: 'loc5', locationName: 'Stock/B2', quantity: 2 },
    ],
  },
  {
    id: '7', name: 'Industrial Valve IV-25', sku: 'VLV-025', category: 'Components',
    unitOfMeasure: 'pcs', stock: 45, location: 'WH/Stock/C2',
    minStockThreshold: 15, reorderFlag: false,
    stockLocations: [
      { warehouseId: 'wh1', warehouseName: 'Main Warehouse', locationId: 'loc7', locationName: 'Stock/C2', quantity: 45 },
    ],
  },
  {
    id: '8', name: 'Rubber Gasket RG-10', sku: 'GAS-010', category: 'Consumables',
    unitOfMeasure: 'pcs', stock: 500, location: 'WH/Stock/D1',
    minStockThreshold: 100, reorderFlag: false,
    stockLocations: [
      { warehouseId: 'wh1', warehouseName: 'Main Warehouse',    locationId: 'loc8', locationName: 'Stock/D1', quantity: 400 },
      { warehouseId: 'wh2', warehouseName: 'Secondary Storage', locationId: 'loc11', locationName: 'Stock/D1', quantity: 100 },
    ],
  },
]);
...
export const useInventoryStore = create<InventoryState>((set, get) => ({
  ...
  addProduct: (p) => {
    ...
    const newProduct = withReorderFlag({ ...p, id, reorderFlag: false });
  ...
  updateProduct: (id, p) => {
    set(s => ({
      products: s.products.map(prod =>
        prod.id === id ? withReorderFlag({ ...prod, ...p }) : prod
      ),
    }));
  },
    ...
    if (status === 'Done' && op.status !== 'Done') {
      ...
      op.products.forEach(opProd => {
        updatedProducts = updatedProducts.map(p => {
          if (p.id !== opProd.productId) return p;
          const newStock = op.type === 'IN'
            ? p.stock + opProd.quantity
            : Math.max(0, p.stock - opProd.quantity);
          return withReorderFlag({ ...p, stock: newStock });
        });
      });
  ...
  addAdjustment: (adj) => {
    ...
    set(s => ({
      ...
      products: s.products.map(p =>
        p.id === adj.productId
          ? withReorderFlag({ ...p, stock: adj.countedQty })
          : p
      ),
    }));
  },
